...
import hello.login.web.session.SessionMagager;
...
public class LoginController {
    ...
    private final SessionMagager sessionMagager;
...
/*    @PostMapping("/login")
    public String loginV2(@Validated @ModelAttribute LoginForm form,
        ...
        // 쿠키에 시간 정보를 주지 않으면 세션 쿠키(브라우저 종료시 모두 종료)
        sessionMagager.createSession(loginMember, response);
        return "redirect:/";
    }*/
...
/*    @PostMapping("/login")
    public String loginV3(@Validated @ModelAttribute LoginForm form,
        ...
        // 로그인 성공 처리
        // 세션이 있다면 있는 세션 반환, 없으면 신규 세션을 생성
...
//        sessionMagager.createSession(loginMember, response);
    ...
    public String loginV4(@Validated @ModelAttribute LoginForm form,
                          BindingResult bindingResult,
                          // requestParam을 넣어줌으로서 비회원이 아이템리스트에 들어가려다가 튕기고
                          // 로그인 창으로 넘어온 후 로그인을 했을때 요청했던 url로 다시 되돌아가는 경로를 만들어 줌
